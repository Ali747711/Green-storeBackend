...
export const sellerLogin = async(req, res) => {
     try {
        ...
        if(email === process.env.SELLER_EMAIL && password === process.env.SELLER_PASSWORD){
            const token = jwt.sign({email}, process.env.JWT_SECRET, {expiresIn: '1hour'})
        ...
        res.json({success: false,  message: `Error in seller login: ${error.message}`})
     }
}
...
export const isSellerAuth = async(req, res) => {
     try {
        ...
        res.json({success: false, message: `Error message in isSellerAuth: ${error.message}`})
     }
}
...
export const sellerLogout = async(req, res) => {
    try {
        ...
        console.log(error.message)
        res.json({success: false, message: `Error message in Seller logout: ${error.message}`})
    }
}